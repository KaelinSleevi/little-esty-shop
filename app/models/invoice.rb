...
# Invoice.from(invoice_items.select('invoice_items.id, max(invoice_items.quantity * invoice_items.unit_price * (bulk_discounts.percentage_discount / 100.0)) as discounted_revenue').joins(item:[merchant: :bulk_discounts]).group("invoice_items.id")).sum('discounted_revenue')